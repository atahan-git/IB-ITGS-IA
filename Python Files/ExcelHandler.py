...
def LoadFromExternalExcelFileWithPath (filePath):
    ...
    while ws.cell(x,1).value != None:
        myName = ws.cell(x,4).value;
        myName += " " + ws.cell(x,5).value
        #stu = None
        #for existingStu in DataHandler.allStudents:
        #    if(existingStu.name == myName):
        #        stu = existingStu;
        #        break;
    ...
    print("Loaded from Internal Excel File Successfuly")
    print("-*-*-");
...
def ExporttoExcelFile ():
    ...
    for act in DataHandler.allActivities:
        ws_activities.cell(1,y).value = str(act.period) + str(act.name);
        x = 2
