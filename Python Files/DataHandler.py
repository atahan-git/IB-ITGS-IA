...
def AssignStudentToActivity (stu : Student, period, index, boolShouldUpdateButtonColors, actName):
    ...
    if (stu.assigned[period] != None):
        if (stu.assigned[period].name == actName):
...
MaxSlotCount = 30
def AssignStudents ():
    print("Brace Yourselves. Hungarian Algorithm is incoming")
...
def CalculateAverages():
    ...
    stuStats =  "Average Choice: " + "{0:.2f}".format (totalChoiceCount / len(allStudents)) + "     Worst Choice: " + str (worst + 1)
